...
const requestBook = async (req, res) => {
  try {
    ...
    const [bookAvailability] = await pool.query("SELECT available_copies FROM books WHERE book_id = ?", [book_id]);
    ...
    const [prevIssued] = await pool.query("Select * from book_issued where student_id = ? and book_id = ? ", [student_id, book_id]);

    if(prevIssued.length > 0){
      let status = prevIssued[0].status;
      if(status === "pending"){
        return res.status(400).json({ message: "You have already requested this book and it's pending" });
    ...
    const [requestResult] = await pool.query("INSERT INTO book_issued (student_id,book_id, status) VALUES (?, ?, ?)", [student_id,book_id, "pending"]);
    ...
    return res.status(201).json({ message: "Book requested successfully", requestId: requestResult.insertId });
...
const approveBookRequest = async (req, res) => {
  ...
  if(!issue_id || !book_id){
    return res.status(400).json({ message: "Issue ID and Book ID are required" });
  }
  ...
  try{
    ...
    const [bookAvailability] = await connection.query("SELECT available_copies FROM books WHERE book_id = ?", [book_id]);
    if (bookAvailability.length === 0) {
      return res.status(404).json({ message: "Book not found" });
    }
    else if (bookAvailability[0].available_copies <= 0) {
    ...
    await connection.query("UPDATE book_issued SET status = ? , issue_date = NOW() , due_date = DATE_ADD(NOW(), INTERVAL 14 DAY) , approved_by = ? WHERE issue_id = ?", ["approved", librarianDetails.id, issue_id]);

    // Decrease the available copies of the book by 1
    await connection.query("UPDATE books SET available_copies = available_copies - 1 WHERE book_id = ?", [book_id]);

    await connection.commit();
    return res.status(200).json({ message: "Book request approved successfully" });
...
const returnBookRequest = async (req, res) => {
  ...
  try {
    ...
    const [issueDetail] = await connection.query("SELECT due_date, status FROM book_issued WHERE issue_id = ?", [issue_id]);
    ...
    if(issueDetail[0].status === "RETURN_INITIATED"){
      return res.status(400).json({ message: "You have already initiated a return request for this book" });
    }else if(issueDetail[0].status !== "APPROVED"){      
      return res.status(400).json({ message: "Invalid request: Book is not currently issued " });
    }
    ...
    await connection.query("UPDATE book_issued SET status = ? , late_return = ? WHERE issue_id = ?", ["RETURN_INITIATED", late_return, issue_id]);

    await connection.commit();
    return res.status(200).json({ message: "Book return request initiated successfully" });
...
const approveReturnRequest = async (req, res) => {
  ...
  try{
    ...
    await connection.query("UPDATE book_issued SET status = ? , return_date = NOW() WHERE issue_id = ?", ["returned", issue_id]);
